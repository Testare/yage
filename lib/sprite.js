...
const template = (templateName, rawState) => _extends({}, assets.templateSprites[templateName], {
    name: `${templateName}-${templateCount = 1 + templateCount}つ` }, rawState);

const init = (_ref) => {
    let { fromTemplate } = _ref,
        rawState = _objectWithoutProperties(_ref, ['fromTemplate']);

    return !fromTemplate ? _extends({
        zFrame: 20
    }, rawState, {
